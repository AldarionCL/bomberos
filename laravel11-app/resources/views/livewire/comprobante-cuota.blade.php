        ...
        <div class="bg-white rounded-lg shadow-lg px-8 py-10 md:w-3/4 sm:w-auto mx-auto">
            <div class="flex items-center justify-between mb-8">
                ...
                <div class="text-gray-700">
                    ...
                    <div class="text-sm">Fecha: {{$cuota->FechaPago ?? ''}}</div>
                    <div class="text-sm">Comprobante #: {{$cuota->id ?? ''}}</div>
                </div>
            </div>
            <div class="border-b-2 border-gray-300 pb-8 mb-8">
                <h2 class="text-2xl font-bold mb-4">Recibo del señor(a):</h2>
                <div class="text-gray-700 mb-2">{{$cuota->user->name ?? ''}}</div>
                <div class="text-gray-700 mb-2">{{$cuota->user->persona->Direccion ?? ''}}</div>
                <div class="text-gray-700 mb-2">{{$cuota->user->persona->Comuna ?? ''}}</div>
                <div class="text-gray-700">{{$cuota->user->email ?? ''}}</div>
            </div>
            <table class="w-full text-left mb-8">
                ...
                <tbody>
                <tr>
                    <td class="py-4 text-gray-700">{{$cuota->TipoCuota ?? ''}}</td>
                    <td class="py-4 text-gray-700">{{$cuota->FechaPago?? ''}}</td>
                    <td class="py-4 text-gray-700">${{$cuota->Recaudado ?? ''}}</td>
                </tr>
